...
export function copyAdversaryTemplate(source) {
 ...
 return {name:raw.name,type:'character',img:raw.img,system,items:(raw.items??[]).map(copyAdversaryItem),ownership:{default:0},prototypeToken:{actorLink:false,texture:{src:raw.img||'icons/svg/mystery-man.svg'}},flags:{'genesys-vtt':{rulesProfile:raw.flags?.['genesys-vtt']?.rulesProfile||'realms-of-terrinoth',adversaryRulesSource:structuredClone(raw.flags?.['genesys-vtt']?.adversaryRulesSource??raw.flags?.['genesys-vtt']?.adversarySource??raw.flags?.['genesys-vtt']?.adversaryTemplate??null),adversaryTemplate:structuredClone(raw.flags?.['genesys-vtt']?.adversaryTemplate??raw.flags?.['genesys-vtt']?.adversarySource??{origin:'Custom Genesys',category:'My templates'})}}};
}
