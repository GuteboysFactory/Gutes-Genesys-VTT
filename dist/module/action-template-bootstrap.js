...
Hooks.once('init',()=>{
 ...
 class ActionTemplateData extends foundry.abstract.TypeDataModel{
  static defineSchema(){return {activation:new StringField({initial:'action',choices:['action','maneuver','incidental','out-of-turn-incidental']}),binding:new StringField({initial:'skill-check',choices:['skill-check','assist','maneuver','custom-check']}),skillId:new StringField({initial:''}),difficulty:new NumberField({initial:2,integer:true,min:0,max:5}),notes:new StringField({initial:''})};}
 }
