...
export function recoveryRoster(scene = canvas.scene) {
  try {
    ...
    return { ready: true, rows: state.entries.filter(e => e.side === 'pc').map(entry => {
      ...
      return { actorRef: entry.actorRef, name: actor?.name ?? entry.label, done: Boolean(record), blocked: !actor || entry.encounterStatus === 'dead', recovered: record?.recovered ?? 0 };
    }) };
...
export async function recover(actorRef, skill, bonus = 0, scene = canvas.scene) {
  requireGm();
  if (!['cool', 'discipline'].includes(skill)) throw new Error('Choose Cool or Discipline.');
  ...
  try {
    ...
    try {
      await foundry.documents.ChatMessage.create({ speaker: { alias: actor.name }, content: `<section><strong>Encounter Recovery · ${skill === 'cool' ? 'Cool' : 'Discipline'}</strong><p>Simple check · Success ${successes} · GM talent bonus ${bonus}</p><p>Strain ${before} → ${after} · Recovered ${before - after}</p></section>` });
